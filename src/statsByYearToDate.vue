<template>
    <div v-if="yearStats">
        <h2>Total games played:</h2>
        {{yearStats.totalGamesPlayed}}

        <h2>Average games played per day:</h2>
        {{formatAverage(yearStats.averageGamesPlayedPerDay)}}

        <h2>Percentage of co-operative games played:</h2>
        {{formatPercentage(yearStats.coOpGamesPlayedPercentage)}} ({{yearStats.coOpGamesPlayedCount}} games), {{yearStats.coOpGameWins}} wins and {{yearStats.coOpGameLoses}} loss

        <h2>Win rates between Hannah and John in versus games:</h2>
        {{formatPercentage(yearStats.winPercentageHannah)}} ({{yearStats.winCountHannah}} games) to Hannah and {{formatPercentage(yearStats.winPercentageJohn)}} ({{yearStats.winCountJohn}} games) to John

        <h2>Most games played in a day:</h2> 
        <div v-for="day in mostGamesPlayedInADay" :key="day.date">
            <h3>{{day.games.length}} games played on {{formatDate(day.date)}}</h3>
            <p v-for="game in day.games" :key="game">
                {{game}}
            </p>
        </div>

        <h2>Most played game:</h2>
        <div v-for="game in yearStats.mostPlayedGames" :key="game.name"> 
        {{game.name}} with {{game.plays}} plays
        </div>
        
        <h2>Complete list of unique games played in {{yearStats.title}}:</h2>    
        <ul>
        <li v-for="game in yearStats.uniqueGamesPlayed" :key="game">{{game}}</li>
        </ul>
    </div>
    <div v-else>
        Loading data from {{datecode}}
    </div>
</template>

<script>

import axios from 'axios'
import moment from 'moment'

export default {
  data () {
    return {
      info: {}
    }
  },
  props: ['datecode'],
  mounted () {
      if (!this.datecode) {
          return
      }
    axios
      .get(`https://boardgames-api.calisaurus.net/api/boardgame/stats/byYear/toDate`)
      .then(response => (this.info = response))
  },
  computed: {
      yearStats: function() {
          return this.info.data
      },
      mostGamesPlayedInADay: function() {
          return this.sortByDate(this.yearStats.mostGamesPlayedInADay)
      }
  },
  methods: {
      formatPercentage: function(number) {
          return Number(number * 100).toPrecision(2) + '%'
      },
      formatDate: function(date) {
          return moment(date).format('Do MMMM')
      },
      formatAverage: function(number) {
        return Number(number).toPrecision(2)
      },
      sortByDate: function(list) {
        return list.sort(function (a, b) {
            const dateA = new Date(a.date)
            const dateB = new Date(b.date)
            return dateA - dateB
        })
      }
  }
}
</script>

<style scoped>
p {
    font-size: 2em;
    text-align: center;
}

* {
    font-family: 'Lato', Helvetica, sans-serif;
}
</style>